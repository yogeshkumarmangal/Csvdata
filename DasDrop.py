...
critical=0;
import datetime
start_time = (datetime.datetime.now()-datetime.timedelta(days=0)).strftime("%d-%b-%Y")
...
start_times = (datetime.datetime.now()-datetime.timedelta(days=0))
...
df4=pd.read_excel('Dashboard.xlsx')
df4['Date']=pd.to_datetime(df4.Date)
df4['Date']=df4['Date'].dt.strftime('%Y-%m-%d')
df4.drop_duplicates(subset='Date',keep='last',inplace=True)
df4.to_csv(r'C:\Users\YOGESH MANGAL\Documents\Rupam Sir Work\Dash\Newproj\myenv\Scripts\dashboardsl/Dashboards.csv',index=False)
